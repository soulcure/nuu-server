...
def send_email(stmp, from_addr, from_password, to_addr, content):
    # cc_addr = 'nuujiegui@163.com'
    msg = MIMEText('Your password is : ' + content, 'plain', 'utf-8')  # 邮件内容
    msg['From'] = _format_addr(from_addr)
    msg['To'] = ",".join(to_addr)
    # msg['Cc'] = _format_addr(cc_addr)
    msg['Subject'] = Header('Forget Password', 'utf-8').encode()  # 邮件标题

    server = smtplib.SMTP_SSL(stmp, 465)  # 默认是25
